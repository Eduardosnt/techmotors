...
router.get('/dashboard', (req: AuthRequest, res: Response) => {
  try {
    ...
    const total_oficinas = (
      db.prepare("SELECT COUNT(*) as c FROM oficinas WHERE status_aprovacao='aprovada'").get() as any
    ).c;
    const pendentes = (db.prepare("SELECT COUNT(*) as c FROM oficinas WHERE status_aprovacao='pendente'").get() as any)
      .c;
    ...
    const receita_total = (
      db.prepare("SELECT COALESCE(SUM(valor_estimado),0) as t FROM agendamentos WHERE status='concluido'").get() as any
    ).t;
...
router.post('/aprovar/:id', (req: AuthRequest, res: Response) => {
  try {
    ...
    db.prepare("UPDATE oficinas SET status_aprovacao='aprovada', aprovado_por=?, aprovado_em=? WHERE usuario_id=?").run(
      req.user!.id,
      now,
      id
    );
...
router.post('/rejeitar/:id', (req: AuthRequest, res: Response) => {
  try {
    ...
    db.prepare("UPDATE oficinas SET status_aprovacao='rejeitada', motivo_rejeicao=? WHERE usuario_id=?").run(
      motivo || '',
      id
    );
...
router.post('/catalogo', (req: AuthRequest, res: Response) => {
  try {
    ...
    db.prepare('INSERT INTO catalogo_servicos (nome, categoria, descricao) VALUES (?, ?, ?)').run(
      nome,
      categoria,
      descricao || ''
    );
...
router.put('/catalogo/:id', (req: AuthRequest, res: Response) => {
  try {
    ...
    db.prepare('UPDATE catalogo_servicos SET nome=?, categoria=?, descricao=?, ativo=? WHERE id=?').run(
      nome,
      categoria,
      descricao || '',
  ...
  try {
    ...
    if (av) {
      db.prepare(
        ...
        WHERE usuario_id=?`
      ).run(av.oficina_id, av.oficina_id, av.oficina_id);
    }
