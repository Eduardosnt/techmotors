...
router.post('/iniciar', (req: AuthRequest, res: Response) => {
  try {
    if (req.user!.tipo !== 'cliente') {
      res.status(403).json({ error: 'Apenas clientes podem iniciar conversas' });
      return;
    }
    ...
    if (!conversa) {
      const result = db.prepare("INSERT INTO conversas (cliente_id, status) VALUES (?, 'bot')").run(clienteId);
      conversa = { id: result.lastInsertRowid, cliente_id: clienteId, status: 'bot' };

      // Mensagem de boas-vindas do bot
      db.prepare("INSERT INTO mensagens (conversa_id, remetente, conteudo) VALUES (?, 'bot', ?)").run(
        conversa.id,
        '👋 Olá! Eu sou o assistente virtual da TechMotors.\n\nComo posso te ajudar? Pergunte sobre agendamentos, preços, oficinas, ou digite **"atendente"** para falar com uma oficina.'
      );
    }

    // Buscar mensagens
    const mensagens = db.prepare('SELECT * FROM mensagens WHERE conversa_id=? ORDER BY criado_em ASC').all(conversa.id);
...
router.post('/mensagem', (req: AuthRequest, res: Response) => {
  try {
    ...
    if (!conversa_id || isNaN(Number(conversa_id))) {
      res.status(400).json({ error: 'conversa_id inválido' });
      return;
    }
    ...
    // Verificar pertencimento: o usuário deve ser o cliente OU a oficina da conversa
    if (tipo === 'cliente' && conversa.cliente_id !== userId) {
      res.status(403).json({ error: 'Acesso negado a esta conversa' });
      return;
    }
    if (tipo === 'oficina' && conversa.oficina_id !== userId) {
      res.status(403).json({ error: 'Acesso negado a esta conversa' });
      return;
    }
    if (tipo === 'admin') {
      res.status(403).json({ error: 'Administradores não participam de conversas' });
      return;
    }
    ...
    db.prepare('INSERT INTO mensagens (conversa_id, remetente, conteudo) VALUES (?, ?, ?)').run(
      conversa_id,
      remetente,
      conteudo.trim()
    );
    ...
    if (conversa.status === 'bot' && tipo === 'cliente') {
      ...
      if (
        ...
        const oficinas = db
          .prepare("SELECT o.usuario_id, o.nome_fantasia FROM oficinas o WHERE o.status_aprovacao='aprovada' LIMIT 10")
          .all() as any[];
        ...
        if (lastBotMsg && lastBotMsg.conteudo.includes('Com qual oficina')) {
          ...
          if (escolhida) {
            ...
            db.prepare(
              "UPDATE conversas SET status='atendente', oficina_id=?, atualizado_em=datetime('now','localtime') WHERE id=?"
...
router.get('/mensagens/:conversa_id', (req: AuthRequest, res: Response) => {
  try {
    ...
    const mensagens = db.prepare('SELECT * FROM mensagens WHERE conversa_id=? ORDER BY criado_em ASC').all(conversaId);
...
router.post('/encerrar/:conversa_id', (req: AuthRequest, res: Response) => {
  try {
    ...
    db.prepare("UPDATE conversas SET status='encerrada', atualizado_em=datetime('now','localtime') WHERE id=?").run(
      conversaId
    );
