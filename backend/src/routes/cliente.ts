...
router.get('/busca', (req: AuthRequest, res: Response) => {
  try {
    ...
    const params: any[] = [];
    if (q) {
      sql += ' AND (o.nome_fantasia LIKE ? OR cs.nome LIKE ?)';
      params.push(`%${q}%`, `%${q}%`);
    }
    if (categoria) {
    ...
    if (cidade) {
      sql += ' AND o.cidade LIKE ?';
      params.push(`%${cidade}%`);
    }

    if (!isNaN(lat) && !isNaN(lng)) {
      sql = sql.replace(
        'SELECT DISTINCT o.*, u.nome, u.telefone',
...
router.post('/agendar', (req: AuthRequest, res: Response) => {
  try {
    ...
    db.prepare('INSERT INTO notificacoes (usuario_id, tipo, titulo, mensagem, link) VALUES (?, ?, ?, ?, ?)').run(
      ...
      `${clienteNome} solicitou um agendamento para ${data} às ${hora}.`,
      '#oficina-solicitacoes'
    );
...
router.post('/cancelar/:id', (req: AuthRequest, res: Response) => {
  try {
    ...
    if (result.changes === 0) {
      res.status(400).json({ error: 'Não pode ser cancelado' });
      return;
    }
    ...
    const clienteNome =
      (db.prepare('SELECT nome FROM usuarios WHERE id=?').get(req.user!.id) as any)?.nome || 'Cliente';
    if (ag) {
      db.prepare('INSERT INTO notificacoes (usuario_id, tipo, titulo, mensagem, link) VALUES (?, ?, ?, ?, ?)').run(
        ag.oficina_id,
        'cancelado',
        '❌ Agendamento cancelado pelo cliente',
        `${clienteNome} cancelou um agendamento.`,
        '#oficina-solicitacoes?status=cancelado'
      );
    }
...
router.post('/veiculos', (req: AuthRequest, res: Response) => {
  try {
    ...
    if (!placa || !marca || !modelo) {
      res.status(400).json({ error: 'Placa, marca e modelo são obrigatórios' });
      return;
    }
    ...
    db.prepare('INSERT INTO veiculos (cliente_id, placa, marca, modelo, ano, tipo) VALUES (?, ?, ?, ?, ?, ?)').run(
      req.user!.id,
      placaNorm,
      marca.trim(),
      modelo.trim(),
    ...
    if (err.code === 'SQLITE_CONSTRAINT_UNIQUE') res.status(400).json({ error: 'Placa já cadastrada no sistema.' });
      ...
      res.status(400).json({ error: 'Erro de conta. Faça logout e entre novamente.' });
...
router.post('/avaliar', (req: AuthRequest, res: Response) => {
  try {
    ...
    const ag = db
      .prepare(`SELECT * FROM agendamentos WHERE id=? AND cliente_id=? AND status='concluido'`)
      .get(agendamento_id, req.user!.id) as any;
    if (!ag) {
      res.status(400).json({ error: 'Agendamento não disponível para avaliação.' });
      return;
    }
    ...
    const transaction = db.transaction(() => {
      ...
      db.prepare(
        ...
        WHERE usuario_id=?`
      ).run(ag.oficina_id, ag.oficina_id, ag.oficina_id);
    });
    transaction();
...
function gerarHorariosDisponiveis(oficina_id: number, data: string, duracao: number = 60): string[] {
  ...
  const regra = db
    .prepare('SELECT hora_inicio, hora_fim FROM disponibilidade WHERE oficina_id=? AND dia_semana=? AND ativo=1')
    .get(oficina_id, dayOfWeek) as any;
  if (!regra) return [];

  const bloqueios = db
    .prepare(
      'SELECT data_inicio, data_fim FROM bloqueios WHERE oficina_id=? AND DATE(data_inicio)<=? AND DATE(data_fim)>=?'
    )
    .all(oficina_id, data, data) as any[];
  const ocupados = db
    .prepare(
      `SELECT data_hora, duracao_minutos FROM agendamentos WHERE oficina_id=? AND DATE(data_hora)=? AND status IN('solicitado','confirmado')`
    )
