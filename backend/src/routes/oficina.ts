...
router.put('/perfil', (req: AuthRequest, res: Response) => {
  try {
    ...
    db.prepare('UPDATE oficinas SET nome_fantasia=?, razao_social=?, logradouro=?, numero=?, bairro=?, cidade=?, uf=?, cep=? WHERE usuario_id=?')
...
router.get('/agenda', (req: AuthRequest, res: Response) => {
  try {
    ...
    const pendentes = (db.prepare(`SELECT COUNT(*) as c FROM agendamentos WHERE oficina_id=? AND status='solicitado'`).get(req.user!.id) as any).c;
...
router.post('/solicitacoes/:id/acao', (req: AuthRequest, res: Response) => {
  try {
    ...
    else if (acao === 'recusar') db.prepare("UPDATE agendamentos SET status='recusado', motivo_cancelamento=? WHERE id=?").run(motivo || '', id);
    else if (acao === 'concluir') db.prepare("UPDATE agendamentos SET status='concluido' WHERE id=?").run(id);
    else if (acao === 'cancelar') db.prepare("UPDATE agendamentos SET status='cancelado', motivo_cancelamento=? WHERE id=?").run(motivo || 'Cancelado pela oficina', id);
    else { res.status(400).json({ error: 'Ação inválida' }); return; }

    // Criar notificação para o cliente
    const ag = db.prepare('SELECT a.cliente_id, o.nome_fantasia FROM agendamentos a JOIN oficinas o ON o.usuario_id=a.oficina_id WHERE a.id=?').get(id) as any;
    if (ag) {
      const notifMap: any = {
        confirmar: { tipo: 'confirmado', titulo: 'Agendamento confirmado! ✅', mensagem: `${ag.nome_fantasia} confirmou seu agendamento.` },
        recusar: { tipo: 'recusado', titulo: 'Agendamento recusado', mensagem: `${ag.nome_fantasia} recusou seu agendamento.${motivo ? ' Motivo: ' + motivo : ''}` },
        concluir: { tipo: 'concluido', titulo: 'Serviço concluído! 🎉', mensagem: `Seu serviço em ${ag.nome_fantasia} foi concluído. Avalie sua experiência!` },
        cancelar: { tipo: 'cancelado', titulo: 'Agendamento cancelado', mensagem: `${ag.nome_fantasia} cancelou seu agendamento.${motivo ? ' Motivo: ' + motivo : ''}` }
      };
      const notif = notifMap[acao];
      if (notif) {
        db.prepare('INSERT INTO notificacoes (usuario_id, tipo, titulo, mensagem, link) VALUES (?, ?, ?, ?, ?)')
          .run(ag.cliente_id, notif.tipo, notif.titulo, notif.mensagem, '#cliente-agendamentos');
      }
    }
    ...
    const transaction = db.transaction(() => {
      ...
      for (const d of dias) {
        if (d.ativo && d.hora_inicio && d.hora_fim) {
          db.prepare('INSERT INTO disponibilidade (oficina_id, dia_semana, hora_inicio, hora_fim, ativo) VALUES (?, ?, ?, ?, 1)')
...
router.post('/bloqueios', (req: AuthRequest, res: Response) => {
  try {
    ...
    db.prepare('INSERT INTO bloqueios (oficina_id, data_inicio, data_fim, motivo) VALUES (?, ?, ?, ?)')
      .run(req.user!.id, data_inicio, data_fim, motivo || '');
...
router.post('/servicos', (req: AuthRequest, res: Response) => {
  try {
    ...
    db.prepare('INSERT INTO oficina_servicos (oficina_id, servico_id, preco_modalidade, preco, duracao_minutos) VALUES (?, ?, ?, ?, ?)')
...
router.get('/metricas', (req: AuthRequest, res: Response) => {
  try {
    const porStatus = db.prepare('SELECT status, COUNT(*) as c FROM agendamentos WHERE oficina_id=? GROUP BY status').all(req.user!.id) as any[];
    const receita = (db.prepare(`SELECT COALESCE(SUM(valor_estimado),0) as total FROM agendamentos WHERE oficina_id=? AND status='concluido'`).get(req.user!.id) as any).total;
    const oficina = db.prepare('SELECT nota_media, total_avaliacoes FROM oficinas WHERE usuario_id=?').get(req.user!.id) as any;
    const mesAtual = new Date().toISOString().substring(0, 7); // YYYY-MM
    const mes = (db.prepare(`SELECT COUNT(*) as c FROM agendamentos WHERE oficina_id=? AND substr(data_hora,1,7)=?`).get(req.user!.id, mesAtual) as any).c;
    const receitaMes = (db.prepare(`SELECT COALESCE(SUM(valor_estimado),0) as total FROM agendamentos WHERE oficina_id=? AND status='concluido' AND substr(data_hora,1,7)=?`).get(req.user!.id, mesAtual) as any).total;
